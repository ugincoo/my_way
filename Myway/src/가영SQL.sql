...
update dMaterials set mater_stock = mater_stock-1 where mater_no = 22 and mater_name != 'No';

select * from porder where member_no = 1 && o_status = 0;
update recommend set recom_view = recom_view+1 where recom_no =1;

select useInfo.mater_name, useInfo.drink_no, useInfo.bread_no, useInfo.veg_no, useInfo.source_no, useInfo.che_no, useInfo.drink_no, useInfo.meat_no
	from (select  d.mater_name, d.mater_no, po.drink_no, po.veg_no, po.bread_no, po.che_no, po.source_no, po.meat_no, po.member_no, d.mater_price
			from Porder po, purchase ph, dMaterials d
			where po.porder_no = ph.porder_no) useInfo
	where useInfo.member_no  = 2 and  (useInfo.mater_no = useInfo.bread_no or
										useInfo.mater_no = useInfo.che_no or
										useInfo.mater_no = useInfo.drink_no or
										useInfo.mater_no = useInfo.veg_no or
										useInfo.mater_no = useInfo.meat_no or
										useInfo.mater_no = useInfo.source_no) 
                                        and useInfo.mater_name != 'No'
	Group by useInfo.mater_no;
    
select po.bread_no, po.drink_no, po.veg_no, po.che_no, po.source_no, po.meat_no, d.mater_name, d.mater_no, ph.purchase_date, d.mater_price
			from porder po, dMaterials d, purchase ph
			where po.member_no = 2 and po.porder_no = ph.porder_no
            and 
			(
				d.mater_no = po.bread_no or
				d.mater_no = po.che_no or
				d.mater_no = po.drink_no or
				d.mater_no = po.veg_no or
				d.mater_no = po.meat_no or
				d.mater_no = po.source_no
			)
            order by ph.purchase_date desc;
select *
	from purchase ph,
		(select searchOd.mater_name, count(searchOd.mater_no), mater_price*count(searchOd.mater_no)
			from (select po.bread_no, po.drink_no, po.veg_no, po.che_no, po.source_no, po.meat_no, d.mater_name, d.mater_no, ph.purchase_date, d.mater_price
					from porder po, dMaterials d, purchase ph
					where po.member_no = 2 and po.porder_no = ph.porder_no
						and 
						(
			...
			where searchOd.purchase_date like '%2023-03%'
			group by searchOd.mater_no) searchday
order by ph.purchase_date;
    
select pSearch.mater_no, pSearch.mater_name, count(pSearch.mater_no), mater_price*count(pSearch.mater_no)
	from (select  ph.purchase_date, d.mater_name, d.mater_no, d.mater_price, po.drink_no, po.veg_no, po.bread_no, po.che_no, po.source_no, po.meat_no, po.member_no
			from Porder po, dMaterials d, purchase ph
			where po.porder_no = ph.porder_no
            ) pSearch
	where  pSearch.purchase_date like "%2023-03%"  and pSearch.mater_name != 'No' and pSearch.member_no = 2
			and (pSearch.mater_no = pSearch.bread_no or
